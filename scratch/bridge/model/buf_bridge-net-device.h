 ...
 */
#ifndef BUF_BRIDGE_NET_DEVICE_H
#define BUF_BRIDGE_NET_DEVICE_H
 ...
 *
 * The BufBridgeNetDevice object is a "virtual" netdevice that aggregates
 * multiple "real" netdevices and implements the data plane forwarding
 * part of IEEE 802.1D.  By adding a BufBridgeNetDevice to a Node, it
 ...
 * \brief a virtual net device that bridges multiple LAN segments
 */
class BufBridgeNetDevice : public NetDevice
{
public:
  /**
  ...
  BufBridgeNetDevice ();
  virtual ~BufBridgeNetDevice ();

  /** 
   * \brief Add a 'port' to a bridge device
   * \param bridgePort the NetDevice to add
   *
   * This method adds a new bridge port to a BufBridgeNetDevice, so that
   ...
   * bridging node you must enable IP on the BufBridgeNetDevice itself,
...
private:
   ...
   */
  BufBridgeNetDevice (const BufBridgeNetDevice &);
   ...
   * \returns
   */
  BufBridgeNetDevice &operator = (const BufBridgeNetDevice &);
